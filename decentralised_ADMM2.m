function [K, Cost, info] = decentralised_ADMM2(G,A,B,M,Q,R,userOpts)
...
% Convert matrix to cell for ADMM
N = size(A,2)/2;
CR = sum(G,1)/2;
CR = (CR(N)-1)/2;
A = mat2cell(A, [2*ones(N,1)], [2*ones(N,1)]);
B = mat2cell(B, [2*ones(N,1)], [1]);
M = mat2cell(M, [2*ones(N,1)], [ones(N,1)]);
Q = mat2cell(Q, [2*ones(N,1)], [2*ones(N,1)]);
R = mat2cell(R, [ones(N,1)], [ones(N,1)]);
% ind = [2*ones(N-(2*CR+1),1); 2*(2*CR+1)];
% A = mat2cell(A, ind, ind);
% B = mat2cell(B, ind, 1);
% M = mat2cell(M, ind, ind/2);
% Q = mat2cell(Q, ind, ind);
% R = mat2cell(R, ind/2, ind/2);
...
for i = 1:n
    gB = blkdiag(gB,B{i}); 
    gM = blkdiag(gM,M{i,i});   % this part was wrong
    gQ = blkdiag(gQ,Q{i,i});
    gR = blkdiag(gR,R{i,i});
    %gR = R{1,1};
    for j = 1:n
...
for i = 1:n                   % local variables in nodes
    % dynamics data
    Node{i}.B = B{i};
    Node{i}.M = M{i,i};    % this part was wrong
    Node{i}.Q = Q{i,i};
    Node{i}.R = R{i,i};
...
%         %Modified ---
%         for k = 1:length(Node{nodeIndex}.CliqueVariables)
%             if isnan(Node{nodeIndex}.CliqueVariables{k})
%                 Node{nodeIndex}.CliqueVariables{k} = zeros(size(Node{nodeIndex}.CliqueVariables{k}));
%             end
%         end
%          for k = 1:length(Node{nodeIndex}.Xi)
%             if isnan(Node{nodeIndex}.Xi{k})
%                 Node{nodeIndex}.Xi{k} = zeros(size(Node{nodeIndex}.Xi{k}));
%             end
%          end
%         %  ---   
