function [K, Cost, info] = dddr(G,A,B,M,Q,R,userOpts)
...
% If the subproblem is infeasible, then the value does not change
...
for iter = 1:opts.maxIter
    ...
    CliqueInfea = 0;
    for k = 1:Nc
        [Node,Edge,Clique{k},sol] = cliqueProblemr(k,Clique{k},Node,Edge,A,opts,iter);
        if sol.problem == 1
    ...
    for i = 1:length(NodeOverInd)
        ...
        [Node{nodeIndex},sol] = nodeProblemr(Node{nodeIndex},A{nodeIndex,nodeIndex},opts,iter);
        if sol.problem == 1
    ...
    for k = 1:length(EdgeRepi)
        ...
        X1   = Node{Eind(1)}.Xi;
        X2   = Node{Eind(2)}.Xi;
        [Edge{Eind(1),Eind(2)},sol] = edgeProblemr(Edge{Eind(1),Eind(2)},A12,A21,X1,X2,opts,iter);
        if sol.problem == 1
    ...
    %K = Z*X^(-1);
    %if ~isempty(find(K == NaN)) || ~isempty(find(K == Inf))
        h2 = -1;
    %else
    %    h2 = max(real(eig(gA - gB*K)));
    %end
